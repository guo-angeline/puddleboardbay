...
**Wobble follow-up 2026-07-18:** owner confirmed the pill is gone but the sheet still wobbled up/down at the scroll extremes (top/bottom), with a video. Root cause was the OTHER half of the wobble: an overscroll at the scroll extremes rubber-banded and chained to the document, which drags a `position:fixed` sheet visually (and toggles the iOS URL bar). Fix: added `overscroll-contain` to the sheet scroller (the exact pattern `/disclaimer` uses and this sheet lacked), so overscroll no longer chains to the page. The item-63 `inset:0`/`transition:none` change had only removed the innerHeight-tracking cause, not the overscroll-chain cause. 344 tests, build clean, `overscroll-behavior:contain` confirmed in the compiled CSS; still owner-verify on a real iPhone (not emulator-reproducible).
