...
- 2026-07-24 [done, NOT DEPLOYED] Item 140: committed search behavior already filters correctly. Added a rendered Playwright guard covering desktop/mobile zero-result search, `Map (0)`, retained Watching content, inline recovery placement, and clearing search while preserving South Bay. Commit `6d22a8c`; verifier PASS, 757 tests and build pass. No production code changed. Automatic preview compiled and generated 381 pages, then hit the pre-existing Vercel root `.next` versus `web/.next` configuration mismatch.

- 2026-07-23 [done, DEPLOYED] Item 139: storage failures can no longer silently regain a blank-app path. The original HomeClient crash was already fixed in `83e78d9`; this close-out adds a TypeScript AST guard over every production `localStorage` access plus a Playwright chaos journey for `/` and `/spot/1`. The verifier caught and closed bypasses through deferred callbacks, computed access, `globalThis`, and destructuring. Commit `f90c3ff`; 753 tests, build, local chaos, and production chaos passed with zero page errors. Deployment `dpl_4ZD84wkUo2hhHh6NfNE2tBbwkHBG`.
...
## 136. [done] Server-rendered real spot content + llms.txt (AI-citable, deployed 2026-07-23, 71b50a1)

Shipped the one-time asset upgrade: the 173 spot pages now serve the full per-spot content as indexable HTML instead of a 155-char caption. `app/spot/[id]/page.tsx` server-renders an sr-only `<article>` with the untruncated `notes` + a facts `<dl>` (water type, region, fee tri-state, a static conditions/tide summary, and dogs/rentals/inspection/power-boats when known), from `ALL_SPOTS` (chokepoint, hidden spots excluded), lat/lng untouched. sr-only for the same reason as the existing h1/nav: the drawer stays the visible UX, this is the same data as indexable text, no new UI. `lib/structured-data.ts` `spotDescription` no longer hard-cuts notes at 155 (that dropped the differentiating facts from the JSON-LD + meta description); new `llmsTxt()` generates `/llms.txt` (a route, `force-static`, zero runtime cost) advertising the dataset region-grouped with per-spot links + facts.

Verified at the SERVED-HTML level (the acceptance's own method), live on prod: spot 134's page carries its full 767-char notes past the old 155 cut; `/llms.txt` lists all 173 spots. Verifier PASS (731 tests, +7; 0 hidden-spot leaks, chokepoint imports, honesty mutation caught, no data/coord change). Lawyer CLEAR (publishing our own data + factual descriptions; its advisory applied, the llms.txt now says "a tide-sensitivity flag where known" rather than asserting tide sensitivity as a verified fact, since that field is known-imperfect on bay spots). Framed as optionality/insurance per its own guardrails: double-counts as long-tail SEO and AI-answer-engine citability on data we already own, never to be graded on DAU.
...
## 120. [done] Mobile map cold-open banner: value prop + "good today" teaser (deployed 2026-07-23, 60542b0)

Shipped the mobile map-tab cold-open surface, folding in item 61's deferred E1. ~82% of traffic is mobile and lands on the Map tab (unlabeled dots, the only descriptive copy was a desktop-only `hidden lg:inline` span), while item 61's good-today list sat one tab away under List, so the one-and-done cohort barely saw the retention moat. A slim strip now pins to the top of the map panel: a value prop ("Paddleboard & kayak spots across California") painted INSTANTLY (static string, never blocked on a fetch), that swaps in place to "Good today: {spot} · {badge} ->" once item 61's already-computed `goodTodaySpots[0]` resolves, tappable into that spot.

**Architecture.** New `MapColdOpenBanner`, `md:hidden`, rendered only on the mobile map tab with a non-empty result set (mutually exclusive with the empty-state overlay; z-[1050] clears Leaflet's zoom control). Reuses `goodTodaySpots[0]`, no new fetch; badge driven by the live `nowPaddleability`, never hardcoded (a Breezy top pick reads "Breezy", with the calm-window still the good-today bar). Session-scoped dismiss (`sessionStorage`, since good-today is a daily-changing answer, a permanent kill would retire the teaser after one early tap). Desktop unaffected (it already has the value prop + list panel).

**Legal CLEAR** (one minor fix applied): the good-today variant co-renders the canonical caveat "Guidance only, not a safety guarantee. Conditions shift fast on the water." verbatim (the value-prop variant makes no claim and carries none); both gates flagged the caveat's 10px gray-400 as below AA, so it now uses `text-[11px] text-(--muted)` (the AA token). **The item-61 list caveat and the conditions-panel foot disclaimer still use 10px gray-400; sweeping those to match is a filed follow-up.** Behind the `map-cold-open-banner` kill switch (default ON, no A/B). New SYSTEM `map_banner_loaded` + INTENT `map_banner_clicked`/`map_banner_dismissed`, new `spot_viewed` source `map_banner`, changelog. Verifier PASS (715 tests, +8; protected files untouched, honest-mutation caught). Design brief by design-lead; `/ship` PRD workflow skipped (schema bug, item 100). **Live-verified at 390px:** banner showed "Good today: Shaver Lake Calm", tap opened the drawer, X dismissed it session-scoped, desktop showed no banner. Confirmed live in the prod bundle.
...
## 103. [done] PROTECTED storm/precip term in evaluateGoodWindow (OWNER-APPROVED, deployed 2026-07-23, 4163e73)

**Owner approved the protected deploy in chat 2026-07-23; shipped via the DEPLOY_APPROVAL flow.** `evaluateGoodWindow` (the one function deciding "good to paddle", feeding the in-app panel, the push cron AND the alert emails) gained a **hard thunderstorm exclusion**: a thunderstorm hour is never part of a good window on any surface, so the product can no longer name a window "good" during lightning. Tight protected diff (`0ebf603`): import `isStormyForecast`, add optional `shortForecast` to `HourlyPeriod`, one eligibility clause `&& !isStormyForecast(period.shortForecast)`; a missing shortForecast reads as not-stormy (the storm-badge contract). Plain rain is NOT excluded; instead a **soft in-app-only label** (`61b244c`, non-protected): `windowRainLabel` marks a wet next-good-window "chance of rain" (30-59%) / "rain likely" (>=60%), never suppressing it, on `NextGoodWindowPanel` only (push/email evaluate via `evaluateGoodWindow`/`findGoodWindow`, not the client-side `getNextWindow`, so the label never reaches them).

Verifier PASS (724 tests, +9: 4 storm-exclusion + 5 windowRainLabel; protected diff tight, storm reaches push+email, rain label does not, honesty mutation caught). Lawyer CLEAR (strictly risk-reducing; framing stays "a good window in the forecast", rain label factual). Deploy: merged to main, DEPLOY_APPROVAL written for the exact sha, `vercel deploy --prod` (the predeploy gate DID fire on the protected path and DEPLOY_APPROVAL released it, verified), confirmed live in the prod bundle, `deployed-prod` tagged, DEPLOY_APPROVAL deleted. In July CA precip is ~0 so this is a no-op until the wet season, which is the point: it is correct and live before the rains. Note: this item's old "the gate would NOT catch this change" caveat is now stale, item 106's fix does gate `web/lib/alerts/*` (re-confirmed at deploy).
...
## 139. [done] P0: one unguarded `localStorage` read blanks the ENTIRE app when storage throws (deployed 2026-07-23, f90c3ff)

**Shipped 2026-07-23.** The named HomeClient crash was already closed by `83e78d9` and deployed in `546b2f6`. The remaining-work note was stale: `ReviewsSection.tsx` and `PaddleNowModal.tsx` were already protected. Item 139 closes with two standing defenses instead: a TypeScript AST guard that rejects production `localStorage` access outside a caught try block, and a Playwright chaos journey that makes every `localStorage` method throw. The verifier found and closed guard bypasses through deferred callbacks, computed access, `globalThis`, and destructuring. Local and production runs kept `/` and `/spot/1` fully usable with PaddleNow hidden and zero uncaught errors.
