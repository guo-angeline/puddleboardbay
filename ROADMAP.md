...
**Two slices shipped 2026-07-17, both DEPLOYED.** (a) Tide fast-fail (8538e40): client aborts the tide fetch at 4s, `/api/tides` per-attempt timeout 6s to 2.5s; prod raw route ~5.5s, was up to 13s. (b) Wind/tide decouple + honesty fix (9268c66): the panel no longer waits on both together (`getConditionsRun` exposes the two source promises), so the wind verdict paints at ~330ms while tide loads independently (measured wind 330ms vs tide 4.5s); and a FAILED tide fetch now says "Tide data is unavailable right now" instead of the false "No tide station near this spot" (tide carries a discriminated ok/no-station/errored outcome). `conditions_loaded` unchanged; `conditions_viewed.had_data` refined (changelog).
**Remaining for this item:** (1) collapse the NWS two-hop by precomputing the `/points` gridpoint per spot at build time (the network trace still shows points then forecast; this is the biggest happy-path win left); (2) a cached same-origin wind path (mirrors the item 52 tide proxy) so a spot's wind is shared across viewers for the cache window; (3) the `conditions_loaded.latency_ms` event already exists, so read the before/after off it. Items (4) render-wind-independently and (5) the honesty message are DONE.
...
- **Verify-loop measurement (2026-07-17): the tide sub-fetch hangs on the proxy's full timeout during NOAA flakiness.** With NOAA flapping (observed 200 then connection-fail `000` within a minute, on both direct curl and the prod proxy), a mobile tidal-spot open painted the wind verdict in **102ms** but the `/api/tides` call took **6.5s locally (one 6s timeout) and ~13s in production (6s timeout + one 6s retry)** before resolving to a 502, after which the panel correctly fell back to the static tide notes. The client (`fetchTides`, `lib/conditions.ts`) passes an `AbortController` signal that is never timer-aborted, so the tide loading state waits the proxy's entire timeout+retry. Good news: wind + verdict are already instant and degradation is graceful (allSettled + static-notes fallback). The fix here is to fast-fail the tide hop so its loading state does not sit for 6 to 13s on a flaky upstream (tighter client/proxy timeout, and reconsider whether the server-side retry should double the user-visible wait), and to serve tides from the precomputed/cached path this item already proposes so a healthy-NOAA window gets cached and a flaky one is masked.
