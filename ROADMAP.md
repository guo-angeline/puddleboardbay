...
**The item 39 pilot (2026-07-16) independently confirmed this screen and widened the blast radius.** Ten spots chosen for reasons unrelated to data quality, and deliberately EXCLUDING every spot item 40 had flagged, came back with a **30-40% material defect rate**. Two blind researchers found the same three: **120 Folsom** ("Beals Point has a ramp" is false; DBW types it "No Facility" while every other Folsom launch is typed "Launch"; the place is real and paddleable, so this is a notes defect, not a fabrication), **63 Berkeley** (pin on the DoubleTree hotel lot, ~410m from the nearest slipway), **84 MLK** (two launches ~2.5km apart merged into one record; coordinate fine, notes wrong). The DBW screen caught 120 on the first spot outside item 40's original sample. **Run both screens across all 142 as a data-quality job.** Note also: 3 of 10 official sources are Cloudflare-blocked (Marin, sccgov, EBRPD), and the path of least resistance around that block is an AI search summary, which is exactly what put spot 79 into production. A sanctioned path to those sources is a prerequisite, not a nicety.

**Checked and contained:** spot 79's fabricated boilerplate did NOT spread. Only spot 1 shares its "9-to-10-foot" tidal figure, and both are South Bay tidal-slough spots where a large range is genuinely expected; 79's other distinctive phrases appear in no other record.
