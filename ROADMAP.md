...
## 18. [blocked(D7)] iOS install loses saved spots: the Safari -> PWA storage partition voids items 13/14

**Blocked on D7 (2026-07-10):** the fix is gated on a real-iOS-device repro (the item's own acceptance: "the repro is the gating experiment before any fix"), which the studio cannot run. D7 asks the owner to run it; if confirmed, the recommended fix is server-side favorites persistence keyed to the anon id (a client-only fix can't cross the storage partition). Answer D7 to unblock.
