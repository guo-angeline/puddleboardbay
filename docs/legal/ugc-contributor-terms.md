...
**0. Why s2.5 is worded the way it is (added v1.3, 2026-07-21, item 83).** The collectables layer gives contributors small private marks, which makes this an incentivised-review program under 16 CFR Part 465. The Part bars incentives conditioned on sentiment **in either direction**, so a mark for "reporting a hazard" would be exactly as non-compliant as a mark for praise; both were considered and the hazard mark was rejected on that ground. The award function in `web/lib/marks.ts` never receives a rating value, so conditioning on sentiment is unwritable rather than merely unwritten, and a test asserts it. Two wording points are deliberate. First, s2.5 conditions on **opinion**, not on content: an earlier draft promised marks "never depend on what you say", which was false, because one mark is earned by writing sentences. Second, the operative clause states the promise plainly and keeps this reasoning here, so a contributor is not handed a clause that argues its own compliance or cites a file path that will move.

**0a. Retroactive award needs no re-acceptance.** Marks were granted for reviews submitted under v1.0 to v1.2. They confer nothing, change no licence term, and cannot have induced a review written before the program existed. That absence of ex-ante inducement is the strongest fact about the 37 pre-existing reviews, and it is why no re-assent was sought.

**0b. Marks stay private as a legal constraint, not a scoping choice.** The day a mark becomes visible to a reader it becomes site-conferred credibility attached to third-party speech, which re-opens the host-versus-co-author question in DECISIONS.md D30 Q1 and adds Part 465 exposure. Any future public display needs its own gate.
...
- **The do-not-sell and do-not-share sentence in 9.2** was VERIFIED 2026-07-21 against the live stack: no advertising, affiliate, or attribution trackers exist anywhere in the app. Third-party origins are PostHog (US cloud), Carto tiles, Formspree, Google Fonts, and Google Maps links. The sentence is accurate. It becomes false the moment any ad or attribution tag is added, so re-check then.
...
1. ~~Register the DMCA designated agent with the U.S. Copyright Office.~~ **DONE 2026-07-21, registration `DMCA-1076005`.** Renewal due by July 2029; the safe harbor lapses if it is missed.
...
4. **Store proof of assent on every submission record:** UTC timestamp, account id, the terms version identifier (`v1.1`), and an archived copy or hash of the exact text displayed. The cap in 10.4 and the waiver in 12.2 bind only the version the contributor actually saw and checked.
5. **Ship the Privacy Policy user-content section in the same release** (D24 requirement). Section 9.1 points at it.
6. ~~Verify the do-not-sell and do-not-share sentence against the live analytics and pixel stack.~~ **DONE 2026-07-21.** Swept `web/app`, `web/components`, `web/lib` for ad and attribution trackers: **none exist.** No Facebook/Meta pixel, no Google Ads or gtag, no TikTok, no AdRoll, no DoubleClick, no Hotjar, no Mixpanel, no Segment. The complete third-party origin list is PostHog (`us.i.posthog.com`, first-party product analytics on US cloud), Carto (map tiles), Formspree (feedback form), Google Fonts, and Google Maps deep links. None is an advertising network and none receives personal information for cross-context behavioral advertising, so the sentence in 9.2 is accurate as written. **Re-verify if any ad, affiliate, or attribution tag is ever added**, since that sentence becomes false the moment one is, which is an FTC Section 5 problem. (Minor, not a sale: Google Fonts discloses the visitor's IP to Google. Self-hosting the fonts would remove that and speed up first paint, worth doing sometime, not a compliance blocker.)
...
**2.5 Marks are for taking part, never for your opinion (added v1.3, 2026-07-21).** We give small marks for contributing, shown only to the contributor and to nobody else. They are awarded on whether someone took part, never on their opinion of a spot or how they rated it, in either direction. Marks have no cash value, buy nothing, cannot be transferred, and do not affect whether a review is published or how it is moderated. We disclose them in the review form itself, before you submit.
...
**5.3 Why we reject a review, and why we do not (amended v1.3, 2026-07-21).** Because taking part earns marks (2.5), our rejection reasons are enumerated rather than open-ended. An open "any reason or no reason" discretion sitting behind a reward is an incentive conditioned on our approval, which 16 CFR Part 465 treats as an implied sentiment condition even where no code reads a rating. We reject a submission only because it: is not based on the contributor's own first-hand experience; has an undisclosed material connection; contains abuse, harassment, or slurs; contains a third party's personal information; accuses someone of a crime or a regulatory violation; is off-topic or spam; or is illegible. **We do not reject a review for being critical or negative, and a rejection never depends on the rating given.** We remain under no obligation to explain an individual rejection. Rejection is not a judgment about the contributor.
...
**6.1 Anything you write is read by a person first.** If your submission contains text, it is held and then either published substantially as written, or rejected in whole. It is never published automatically. We do not rewrite the substance of user submissions.

**A star rating submitted on its own, with no review text, appears immediately, and it is published with no name attached to it.** There is nothing written for a person to read. We may remove any rating at any time under Section 6.3. To have your own rating taken down, email hello@paddletowater.com, or delete your account under Section 9.3. (Amended in v1.1, 2026-07-21. The rationale: pre-publication review exists to catch defamatory, unlawful, or abusive *words*. A bare numeric rating carries none of that risk, and holding it added delay without adding safety.)

**6.2 We are a host, not the author.** We act as a host and distributor of Content created by users, not as its author or publisher. Reviews reflect the views of the contributors who wrote them, not the views of Paddle to Water or its owner. Our decision to publish, reject, or later remove a review does not make us the author or publisher of that Content, and we do not adopt or endorse it. The score shown for a spot is computed by us. It combines our own rating of the spot, weighted as five contributor reviews, with every published contributor rating (amended v1.2, 2026-07-21, when the blended score shipped). It is therefore our statement, not the contributors', and it is not a statement that a spot is safe. Where a spot has no rating of our own, the number shown is a plain average of contributor ratings. See the standing escalation trigger below on adding editorial voice: this change is an instance of it, recorded in DECISIONS.md as the D24 amendment.
...
**3. The DMCA safe harbor is now yours, but it expires.** Registered 2026-07-21 as `DMCA-1076005`. This closes the one gap Section 230 never covered, since Section 230 does not shield copyright claims. **It lapses in three years.** Put a reminder for **July 2029**. If you miss the renewal, the safe harbor disappears and you will probably not notice until someone sends a claim.
