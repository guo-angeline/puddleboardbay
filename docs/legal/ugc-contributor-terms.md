...
**2.5 Marks are for taking part, never for what you say (added v1.3, 2026-07-21).** We give small marks for contributing, shown only to the contributor and to nobody else. They are awarded on whether someone took part, never on what they wrote or how they rated a spot, in either direction: 16 CFR Part 465 bars incentives conditioned on sentiment both ways, so a mark for reporting a hazard would be as non-compliant as a mark for praise. Marks have no cash value, buy nothing, and do not affect whether a review is published. The award function does not receive a rating value at all (`web/lib/marks.ts`), so conditioning on sentiment is unwritable rather than merely unwritten. The disclosure is shown in the review form itself, where the incentive is acting on the reader.
