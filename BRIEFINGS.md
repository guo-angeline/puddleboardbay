...
Evidence: Verifier PASS on all 8 checks (no anchor in the paddle-now path, no spot list shown without a real location, full caveat guarded, location request + all three outcomes handled, once-per-day flag still written on render, dead code fully removed, a11y intact). 746 tests (the old 60-minute-logic tests were removed with the logic; usePaddleNow hook + paddleNow helpers deleted). Verified LIVE desktop + mobile: the ask step renders with the new copy; granting a (mocked) location applied it and the list showed real distances (3.2 mi, 5.5 mi, not a central-CA ranking); the honest empty state ("Nothing near you is calm enough to paddle today") rendered because it is past 6pm and nothing is calm today for anyone (item 61's panel agrees). Confirmed live in the prod bundle: new copy present, old "good to go" copy gone, privacy page updated.
Legal: Lawyer CLEAR. The modal now actively requests geolocation, but it is the same permission and client-only handling as the existing "Near me" button (no coordinates logged or sent; the two events carry only a boolean and an enum), and the ask is the just-in-time, consent-forward pattern regulators favor. Applied its one optional note: broadened the /privacy geolocation copy from naming only "Near me" to covering any in-app location request.
P0 caught and fixed in flight: while merging, a verify-loop finding (now item 139) landed in ROADMAP naming an unguarded localStorage.getItem in this exact effect that blanks the ENTIRE app when storage throws (Safari Private Browsing, quota, disabled storage). Since the redesign rewrote that effect, I wrapped the read in try/catch as part of this deploy (treat unreadable storage as "seen", modal stays hidden). That closes the app-blanking crasher; item 139 stays [ready] for the remaining ReviewsSection read + a standing lint/test guard.
Deployed: paddletowater.com, 2026-07-23 (546b2f6), confirmed live. The merge preserved the concurrently-landed Item 100 "today's shape" fix and the verifier's uncommitted item 139 finding (both intact on main).
Analytics: paddle_now_shown drops `count` (the primer renders before a spot count) and now counts the ask impression; new paddle_now_located(outcome: granted|denied|unsupported) for the location ask. Changelog documents the props-change and the comparability break (paddle_now_shown is discontinuous from the redesign).
Decisions raised: none. D36 (monetization) and D37 (retention read) still await the owner.
Next up: item 139 [ready] (finish the localStorage-throw sweep + guard). The rest of the [ready] queue is otherwise dry with 14 [proposed] shelved.

## 2026-07-23 · Item 138: SoCal inland coverage, tranche 1 (5 reservoirs + Inland Empire region) · SHIPPED
What: The owner asked to raise thin SoCal coverage. The real gap (per the 2026-07-22 source inventory) was never the coast, which items 90/94/95/96 already mined; it was the population-dense INLAND reservoirs east and north of LA, which had zero coverage. This tranche adds the first five: Lake Perris, Silverwood Lake, Castaic Lagoon, Big Bear Lake and Pyramid Lake (ids 185-189), plus a new "Inland Empire" region. High-value flatwater close to the LA / Inland Empire user base, and inland lakes sidestep the tide_sensitive landmine (false from fact).
Method (the proven items-90/94/95/96 way, not the spot-79 way): an operator or registry generates the candidate; a search never produces a record. Every put-in coordinate is an OSM-verified slipway, not a geocoded address (the address-to-road trap is exactly why Lake Elsinore was deferred: its 32040 Riverside Drive geocoded to a road point). Several OSM slipways were named identically to the operator's own ramp (Castaic "Lagoon Launch Ramp", Big Bear "Carol Morrison East Boat Ramp", Pyramid "Emigrant Landing Boat Ramp"). Public paddle access was confirmed per site against the operating agency (parks.ca.gov, Big Bear MWD, Riverside County, Angeles NF concessionaire), with permits/inspections/fees written into the evergreen notes. Per-field provenance throughout.
Discipline shown by what was NOT ingested: Lake Skinner and Diamond Valley Lake are excluded (MWD drinking-water rules ban body contact and SUP, allow only 10ft+ sit-inside kayaks), and Lake Cahuilla is excluded (sources conflict on whether paddling is allowed at all). Excluding is not hiding: a DBW/registry hit is never grounds to hide, but an unconfirmed or SUP-hostile lake is not a paddle spot this app should assert. All classifications are in reports/socal-inland-ingest-2026-07-23.md (CARRIED / MERGED / GENUINE-GAP / EXCLUDED / DEFERRED).
Evidence: Verifier PASS on all 9 data-integrity checks (no existing-coordinate churn, 16-field completeness, all 5 gridpoints resolve, region ADDED not renamed, tide_sensitive false, no em dashes, power_boats:false correct for the non-powered Castaic Lagoon, coordinates plausible on the named lakes). 747 tests, build 381 static pages incl. the 5 new spot pages. Verified LIVE on paddletowater.com: all five /spot pages return 200, spot 188's served HTML carries "Big Bear Lake" + "Inland Empire" + the notes, all five are in the sitemap, and the Big Bear conditions panel loaded real NWS wind (the gridpoint landmine did not fire on the new region). The "Inland Empire" filter pill renders in the correct position and filters to exactly its three spots.
Legal: Lawyer CLEAR. Same posture as item 136: publishing our own factual descriptions of public facilities, sourced to the operating agencies, under the app-wide safety caveat. OSM coordinates are ODbL and the base map already renders the required "(c) OpenStreetMap contributors" credit; 5 lat/lng values are de-minimis factual data. One minor optional note (keep permit/fee facts fresh; the notes correctly avoid dollar figures), no action required.
Process note worth the board's attention: the verifier caught a STALE-WORKTREE revert. My worktree forked before a concurrent process landed the Item 100 "today's shape" fix on main; deploying the worktree directly would have reverted that live fix (the exact 2026-07-15 failure mode). Because item 138 does not touch todaysShape, MERGING the branch into current main preserved the fix, and I deployed from main's tree, not the worktree, then re-verified the fix survived (todaysShape guard still `< 3`) before tagging deployed-prod. This deploy also carried the Item 100 fix live (it had been committed but not yet deployed).
Deployed: paddletowater.com, 2026-07-23 (6749d1d), confirmed live.
Decisions raised: none. D36 [OPEN] (monetization) and D37 [OPEN] (retention read compromised) still await the owner.
Follow-ups: the deferred Lake Elsinore + Lake Gregory (access confirmed, put-in needs a second source) and the San Diego County inland reservoirs (Miramar, Murray, Hodges, Poway, Santee, Jennings, Otay, San Vicente, El Capitan, Dixon) are the obvious next inland tranche. Item 45 stays open for the un-registried regions. Not filed as a new item; the owner can promote when coverage is the priority again (retention remains the stated lead).
Next up: the [ready] queue is now dry after this ship.

## 2026-07-23 · Item 137: First-visit-per-day "Want to paddle now?" modal · SHIPPED
What: The owner's push variant of the conditions moat. On a user's first home visit each local day, a closable ~half-screen dialog opens titled "Want to paddle now?" listing up to 3 nearest spots whose calm window overlaps the next 0-60 minutes, each tap-through to its drawer. Where item 61 is a passive pull surface (a pinned "good today" list) and item 120 a mobile map teaser, this is the interruptive, tighter-window (now to +60min) prompt: the first thing a returning paddler sees is a direct answer to "can I go right now, and where?". Aimed squarely at the 78% one-and-done retention problem.
Architecture: New pure web/lib/paddleNow.ts reuses evaluateGoodWindow via evaluateGoodToday + the shared getHourlyPeriods cache, so it never forks the "good to paddle" math and never contradicts the drawer it opens into or the alert cron; the ONLY new constraint is the horizon filter (window start within [now, now+60min]). Candidate set = item 61's nearest-K fan-out with the GOOD_TODAY_ANCHOR fallback when geolocation is denied, excluding watched/recently-checked. SUPPRESSES entirely unless >=1 good-soon spot resolves, so it never opens to say "nothing". Once-per-local-day localStorage gate written ON genuine render (inside the modal's mount effect), so a quiet day never burns the daily showing. Home-only: never on a deep-link arrival.
Evidence: Adversarial verifier PASS on all 8 acceptance criteria, 747 tests (+16), tsc/lint clean of item files (two pre-existing errors elsewhere, not from this change), no em dashes, no coordinate changes, protected files untouched. Verifier ran its own horizon boundary probe against the real modules (+60min inclusive-good, +61min not-good, window-in-progress picks the next future hour). Live-verified desktop 1280px + mobile 390px on real NWS data: modal rendered with the correct not-located lead, close/backdrop/Escape all dismiss and set the flag, same-day reload suppressed, spot-tap opened the drawer, and the modal's "Calm now" matched the drawer's Right-Now verdict (6 mph) for the same spot. The empty-suppression path was confirmed correct (when the only good-soon spots were all recently-checked, the modal correctly did not open). Confirmed live in the prod bundle on paddletowater.com.
Legal: Lawyer gate (required, affirmative "go now" claim on a new insistent surface) returned CLEAR. The canonical caveat "Guidance only, not a safety guarantee. Conditions shift fast on the water." co-renders verbatim, guarded by a test. Its one minor note, that on this more-insistent auto-opening surface the caveat should not read as fine print, was applied: bumped from text-xs/muted to body size/primary ink so the qualifier reads at the weight of the claim. No PII: the three analytics events carry no coordinates; `located` is a boolean derived from the pre-existing Near Me grant, and localStorage holds only a local date string.
Measure: New INTENT paddle_now_shown (count, located) + paddle_now_spot_clicked (spot_id, region, rank) + paddle_now_dismissed (method: close|backdrop|escape|spot_click), added to the union + EventPropMap, changelog filed. Behind the paddle-now kill switch, default ON, no A/B per DAU<100. The dismiss-method split (close/backdrop/escape vs spot_click) is the interruption-cost read: a high non-click share means the prompt is spam, and the kill switch is the reversibility lever. n/a on pre-post until data lands.
Open question resolved: whether to show on mobile (where the app cold-opens on Map and item 120 already teases) defaulted to yes, show on both; live mobile check confirmed the once-a-day modal and item 120's persistent banner don't stack awkwardly (modal is a one-time gate, banner is ambient).
Deployed: paddletowater.com, 2026-07-23 (4edc143), confirmed live in the prod bundle.
Decisions raised: none from this item. D34 [OPEN] (re-baseline retention onto the pull channel) and D35 [OPEN] (freeze-or-commit native) still await owner.
Process: /ship PRD workflow skipped again (same StructuredOutput schema failure); pipeline composed by hand (data layer TDD, then verifier + lawyer in parallel). Merge into main was disjoint from the concurrent studio/strategy state commits and clean; deployed-prod moved to the verified-live merge; foreign stash (studio-state D1+proposals) left untouched per the hard rules.
Next up: item 138 [ready] (owner-directed SoCal coverage expansion). It is a spot-data item, so it carries the full new-geography checklist (gridpoints, TIDE_STATIONS in range, provenance-not-precision, dedupe by identity); the next loop fire picks it up.
...
## 2026-07-23 · Item 136: server-rendered spot content + llms.txt (AI-citable) · SHIPPED
What: The 173 spot pages served a 155-char caption of each spot to crawlers and AI answer engines, while the rich per-spot data (full notes, fee, water type, tide) lived only in the client-only drawer. Now the served HTML carries the full article: an sr-only server-rendered block with the untruncated notes + a facts list, the 155-char truncation removed from the JSON-LD/meta description, and a new /llms.txt advertising the whole dataset to AI engines. No new UI (sr-only, drawer unchanged), no data change, lat/lng untouched.
Evidence: Verifier PASS, 731 tests (+7), 0 hidden-spot leaks, chokepoint imports, honesty mutation caught. Lawyer CLEAR (publishing our own data + factual descriptions); its advisory applied, the llms.txt says "a tide-sensitivity flag where known" not "tide sensitivity" as a verified fact, since that field is known-imperfect on bay spots. Verified live on prod at the served-HTML level: spot 134's page carries its full 767-char notes past the old cut; /llms.txt lists all 173 spots.
Framing: optionality/insurance per its own guardrails (double-counts as long-tail SEO + AI-answer-engine citability on data we already own), never to be graded on DAU. Deployed paddletowater.com 2026-07-23 (71b50a1).
Next up: item 137 (the owner's "Want to paddle now?" first-visit modal), then 138 (SoCal coverage).
