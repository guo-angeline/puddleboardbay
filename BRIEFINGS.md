...
NOT deployed: production is unchanged. The autonomous `vercel --prod` was gated by the auto-mode classifier (converting to 100% touches the "never straight to 100%" directive, even though D2(a) is the owner's explicit exception). **Owner: run `vercel --prod --yes` from the repo root to ship.** Only after deploy does the 100% rollout + decontaminated metric take effect.
