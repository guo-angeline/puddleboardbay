...
- **The `t` (subscription token) param is now stripped from the URL via `history.replaceState`** before PostHog initializes, so `$current_url` stops carrying `t=` from this date. Any historical URL-property analysis spans a discontinuity, and pre-deploy rows contain live tokens (see `queries/token_leak_check.sql`).
