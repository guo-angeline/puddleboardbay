...
- **The `t` (subscription token) param must be stripped from the URL via `history.replaceState`, after the email-open ping fires and before `$current_url` is captured, to close the pre-existing leak documented in `queries/token_leak_check.sql` (D18 legal gate action 2).** As of this entry that strip has not landed in `components/HomeClient.tsx`: `$current_url` still carries a live `t=` value on every email arrival. Do not read a `$current_url` discontinuity for `t=` as starting on this deploy date. It starts on whichever date the strip actually ships, which must be confirmed before that date is treated as the cutover in `queries/token_leak_check.sql`.
